...
void *status_update(int socket_id)
{
    ...
    {
        redis_mtx.lock();
        redisHandler->get_key(std::to_string(socket_id) + ':' + _player_connected);
...
void *client_listener(void *arg)
{
    ...
    redis_mtx.lock();
    redisHandler->set_key(std::to_string(socket_id) + ':' + _player_connected, std::to_string(1));
    redis_mtx.unlock();
    ...
    while (connected)
    {
        activity = select(3, &readfds, NULL, NULL, NULL);
...
void wait_for_connection(pthread_t *client_threads, int &master_socket)
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < max_clients; i++)
        {
            ...
            if (client_socket[i] == 0)
            {
                ...
                int *socket_id = new int;
                *socket_id = client_count;
                pthread_create((client_threads + client_count), NULL, client_listener, static_cast<void *>(socket_id));
                // pthread_create(*(client_threads + client_count)
...
int main(int argc, char *argv[])
{
    ...
    wait_for_connection(client_threads, master_socket);
    // int client_count = 0;
    // while (true)
    // {
    //     FD_ZERO(&readfds);
